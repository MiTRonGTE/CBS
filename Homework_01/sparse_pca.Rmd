...
```{r}
pdf("problem02_KP.pdf", height=7, width=10)
...
```{r}
pdf("problem03_KP.pdf", height=7, width=10)
...
pdf("problem04_KP.pdf", height = 7, width = 10)
LSV5 <- data.table(svd.out$u[, 1:5])
LSV5 <- gather(LSV5, key="LSV_number", value="Values")
ggplot(LSV5, aes(x=0, y=Values)) + geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) + facet_grid(~LSV_number) + xlab(" ") + ylab("Vector values") + 
